...
public class PlayerListener implements Listener {
					...
					if (tPlayer.isDead()){
						...
						for (Body b : Main.bodies){
							if (b.getLocation().equals(Location2i.getLocation(e.getClickedBlock()))){
								...
								e.getPlayer().sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("discreet"));
						...
						if (index != -1){
							...
							if (!found){ // it's a new body
								for (Player p : e.getPlayer().getWorld().getPlayers()){
									if (Main.bodies.get(index).getPlayer().getRole() == Role.INNOCENT)
										p.sendMessage(ChatColor.DARK_GREEN + e.getPlayer().getName() + " " +
												Main.local.getMessage("found-body").replace("%",
														Main.bodies.get(index).getPlayer().getName())  + ". " +
														Main.local.getMessage("was-innocent"));
									else if (Main.bodies.get(index).getPlayer().getRole() == Role.TRAITOR)
										p.sendMessage(ChatColor.DARK_RED + e.getPlayer().getName() + " " +
												Main.local.getMessage("found-body").replace("%",
														Main.bodies.get(index).getPlayer().getName())  + ". " +
														Main.local.getMessage("was-traitor"));
									else if (Main.bodies.get(index).getPlayer().getRole() == Role.DETECTIVE)
										p.sendMessage(ChatColor.DARK_BLUE + e.getPlayer().getName() + " " +
												Main.local.getMessage("found-body").replace("%",
														Main.bodies.get(index).getPlayer().getName())  + ". " +
														Main.local.getMessage("was-detective"));
								}
								Main.foundBodies.add(Main.bodies.get(index));
								Main.bodies.get(index).getPlayer().setBodyFound(true);
								LobbyManager.updateSigns(Main.bodies.get(index).getPlayer().getWorld());
							}
			...
			if (e.getPlayer().getItemInHand() != null){
				if (e.getPlayer().getItemInHand().getItemMeta() != null){
					if (e.getPlayer().getItemInHand().getItemMeta().getDisplayName() != null){
						if (e.getPlayer().getItemInHand().getItemMeta().getDisplayName()
								.endsWith(Main.local.getMessage("gun"))){
							if ((isPlayer(e.getPlayer().getName()) &&
								...
								if (e.getPlayer().getInventory().contains(Material.ARROW) ||
									...
									e.getPlayer().sendMessage(ChatColor.RED +
											Main.local.getMessage("need-ammo"));
							}
							return;
						}
					}
				...
				if (l.getX() == e.getClickedBlock().getX() && l.getY() == e.getClickedBlock().getY() &&
					...
					if (e.getAction() == Action.LEFT_CLICK_BLOCK && e.getPlayer().isSneaking() &&
						...
						e.getPlayer().sendMessage(ChatColor.GREEN + "[TTT] " +
								Main.local.getMessage("lobby-unregister"));
					}
						...
						e.getPlayer().sendMessage(ChatColor.RED +
								"[TTT] " + Main.local.getMessage("no-permission"));
						e.setCancelled(true);
					}
				}
			}
	...
	public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent e){
		if (e.getMessage().startsWith("kit")){
			if (isPlayer(e.getPlayer().getName())){
				e.setCancelled(true);
				e.getPlayer().sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("no-kits"));
			}
		}
			...
			if (isPlayer(p)){
				e.setCancelled(true);
				e.getPlayer().sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("no-pm"));
			}
		}
	}
						...
						if (damager.getItemInHand() != null)
							if (damager.getItemInHand().getItemMeta() != null)
								if (damager.getItemInHand().getItemMeta().getDisplayName()
										!= null)
									if (damager.getItemInHand().getItemMeta().getDisplayName()
											.endsWith(Main.local.getMessage("crowbar")))
			...
			if (isPlayer(p.getName())){
				...
				if (actualDamage >= ((Player)e.getEntity()).getHealth()){
					if (t.getRole() != null){
						e.setCancelled(true);
						p.setHealth(20);
						p.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("dead"));
						t.setDead(true);
						if (e instanceof EntityDamageByEntityEvent)
						...
						idMeta.setDisplayName(Main.local.getMessage("id"));
						List<String> idLore = new ArrayList<String>();
						idLore.add(Main.local.getMessage("body-of"));
						...
						if (t.getRole() == Role.INNOCENT){
							...
							tiMeta.setDisplayName("�2" + Main.local.getMessage("innocent"));
							List<String> tiLore = new ArrayList<String>();
							tiLore.add(Main.local.getMessage("innocent-id"));
							tiMeta.setLore(tiLore);
						}
						else if (t.getRole() == Role.TRAITOR){
							ti.setDurability((short)14);
							tiMeta.setDisplayName("�4" + Main.local.getMessage("traitor"));
							List<String> lore = new ArrayList<String>();
							lore.add(Main.local.getMessage("traitor-id"));
							tiMeta.setLore(lore);
						}
						else if (t.getRole() == Role.DETECTIVE){
							ti.setDurability((short)11);
							tiMeta.setDisplayName("�1" + Main.local.getMessage("detective"));
							List<String> lore = new ArrayList<String>();
							lore.add(Main.local.getMessage("detective-id"));
							tiMeta.setLore(lore);
						}
	...
	public void onPlayerDropItem(PlayerDropItemEvent e){
		if (isPlayer(e.getPlayer().getName())){
			e.setCancelled(true);
			e.getPlayer().sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("no-drop"));
		}
	}
	...
	public void onPlayerJoin(PlayerJoinEvent e){
		...
		if (e.getPlayer().hasPermission("ttt.build.warn"))
			if (Main.stability.equals("unstable"))
				e.getPlayer().sendMessage(ChatColor.DARK_RED + "[TTT] " + Main.local.getMessage("unstable-build"));
			else if (Main.stability.equals("unknown"))
				e.getPlayer().sendMessage(ChatColor.DARK_RED + "[TTT] " + Main.local.getMessage("unknown-build"));
			else if (Main.stability.equals("pre"))
				e.getPlayer().sendMessage(ChatColor.DARK_RED + "[TTT] " + Main.local.getMessage("prerelease"));
	}
	...
	public void onPlayerQuit(PlayerQuitEvent e){
		...
		if (isPlayer(p)){
			...
			for (Player pl : Main.plugin.getServer().getWorld("TTT_" + worldName).getPlayers())
				pl.sendMessage(ChatColor.DARK_PURPLE + "[TTT] " + p + " " + Main.local.getMessage("left-game")
						.replace("%", worldName));
		}
		if (!Variables.KARMA_PERSISTENCE)
	...
	public void onPlayerTeleport(PlayerTeleportEvent e){
		...
		if (isPlayer(p)){
			if (!e.getFrom().getWorld().getName().equals(e.getTo().getWorld().getName())){
				...
				RoundManager.resetPlayer(e.getPlayer());
				for (Player pl : Main.plugin.getServer().getWorld(worldName).getPlayers())
					pl.sendMessage(ChatColor.DARK_PURPLE + "[TTT] " + p + " " + Main.local.getMessage("left-game")
							.replace("%", worldName));
			}
		}
