				...
				if (playerCount >= Variables.MINIMUM_PLAYERS){
					if((currentTime % 10) == 0 && currentTime > 0){
						for (Player p : Main.plugin.getServer().getWorld("TTT_" + worldName).getPlayers()){
							p.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("begin")
									.replace("%", currentTime + " " + Main.local.getMessage("seconds") + "!"));
						}
					}
						...
						int players = Main.plugin.getServer().getWorld("TTT_" + worldName).getPlayers().size();
						int traitorNum = 0;
						...
						for (Player p : Main.plugin.getServer().getWorld("TTT_" + worldName).getPlayers()){
							...
							p.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("begun"));
						}
						...
						dnaMeta.setDisplayName("§1" + Main.local.getMessage("dna-scanner"));
						...
						for (String s : innocents){
							...
							if (pl != null && t != null){
								...
								pl.sendMessage(ChatColor.DARK_GREEN + Main.local.getMessage("you-are-innocent"));
						...
						for (String s : traitors){
							...
							if (pl != null && t != null){
								...
								pl.sendMessage(ChatColor.DARK_RED + Main.local.getMessage("you-are-traitor"));
								if (traitors.size() > 1){
									pl.sendMessage(ChatColor.DARK_RED + Main.local.getMessage("allies"));
									...
									pl.sendMessage(ChatColor.DARK_RED + Main.local.getMessage("alone"));
								pl.getInventory().addItem(new ItemStack[]{crowbar, gun, ammo});
								pl.setHealth(20);
								pl.setFoodLevel(20);
							}
						...
						for (String s : detectives){
							...
							if (pl != null && t != null){
								...
								pl.sendMessage(ChatColor.BLUE + Main.local.getMessage("you-are-detective"));
						...
						if (Variables.DAMAGE_REDUCTION){
							for (Player p : Main.plugin.getServer().getOnlinePlayers()){
								if (TTTPlayer.isPlayer(p.getName())){
									...
									String percentage = Main.local.getMessage("full");
									if (t.getDamageReduction() < 1)
										percentage =
										Integer.toString((int)(t.getDamageReduction() * 100)) + "%";
									p.sendMessage(ChatColor.DARK_PURPLE +
											Main.local.getMessage("karma-damage")
