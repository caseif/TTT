...
public class LobbyManager {
	...
	public static void addSign(Block b, String world, String type, int number, Player p){
		if (b.getState() instanceof Sign){
			...
			if (type.equalsIgnoreCase("status")){
				...
				signs.add(l);
				updateSigns(world);
				p.sendMessage(ChatColor.GREEN + "[TTT] " + Main.local.getMessage("lobby-create"));
			}
				...
				if (number > 0){
					...
					p.sendMessage(ChatColor.RED + Main.local.getMessage("invalid-sign"));
			}
			else
				p.sendMessage(ChatColor.RED + Main.local.getMessage("invalid-sign"));
		}
							...
							if (b.getState() instanceof Sign){
								...
								if (s.getType().equals("status")){
									...
									if (status.equals("WAITING"))
										status = Main.local.getMessage(status.toLowerCase() + "-sign");
									else
										status = Main.local.getMessage(status.toLowerCase());
					...
					if (b.getState() instanceof Sign){
						...
						if (s.getType().equals("status")){
							...
							if (!max.equals("∞") && 0 >= Integer.parseInt(max))
								...
								playerCount = "§a" + playerCount;
							sign.setLine(1, playerCount);
							String status = "§7" + Main.local.getMessage("waiting-sign");
	...
	public static void removeSign(LobbySign s){
		try {
			...
			File f = new File(Main.plugin.getDataFolder(), "signs.yml");
			y.load(f);
			...
			ex.printStackTrace();
			Main.log.warning(Main.local.getMessage("lobby-unregister-fail"));
		}
	}
	...
	public static int saveSign(LobbySign l){
		...
		try {
			...
			ex.printStackTrace();
			Main.log.warning(Main.local.getMessage("lobby-create-fail"));
		}
		return nextKey;
	}
