...
public class RoundManager {
	...
	public void gameTimer(final String worldName){
		...
		tasks.put(worldName, Main.plugin.getServer().getScheduler().runTaskTimer(Main.plugin, new Runnable(){
			...
			public void run(){
				...
				for (TTTPlayer tp : players){
					if (tp.getWorld().equals(worldName)){
						if (Round.getRound(worldName) != null){
							Player p = Main.plugin.getServer().getPlayer(tp.getName());
							if (p != null){
								if (!Main.plugin.getServer().getWorld("TTT_" + worldName).getPlayers().contains(p)){
									if (checkPlayers.contains(tp.getName())){
										if (Variables.VERBOSE_LOGGING)
											Main.log.info(tp.getName() +
										...
										Bukkit.broadcastMessage("[TTT] " + tp.getName() + " " +
												Main.local.getMessage("left-map") + " \"" + worldName + "\"");
									}
									else
										checkPlayers.add(tp.getName());
								}
								else if (tp.getRole() == Role.DETECTIVE){ // manage DNA Scanners
									Player tracker = Main.plugin.getServer().getPlayer(tp.getName());
									if (tp.getTracking() != null){
										Player killer = Main.plugin.getServer().getPlayer(tp.getTracking());
				...
				if (!(tLeft && iLeft)){
					...
					if (rTime % 60 == 0 && rTime >= 60){
						for (Player p : Main.plugin.getServer().getWorld("TTT_" + worldName)
								.getPlayers()){
							p.sendMessage(ChatColor.DARK_PURPLE + Integer.toString(rTime / 60) +
									" " + Main.local.getMessage("minutes") + " " +
									Main.local.getMessage("left"));
						}
					}
						...
						for (Player p : Main.plugin.getServer().getWorld("TTT_" + worldName)
							...
							p.sendMessage(ChatColor.DARK_PURPLE + Integer.toString(rTime) + " " +
									Main.local.getMessage("seconds") + " " +
									Main.local.getMessage("left"));
						}
					}
					else if (rTime < 10 && rTime > 0){
						for (Player p : Main.plugin.getServer().getWorld("TTT_" + worldName).getPlayers()){
							p.sendMessage(ChatColor.DARK_PURPLE + Integer.toString(rTime) + " " +
									Main.local.getMessage("seconds") + " " +
									Main.local.getMessage("left"));
						}
				...
				for (TTTPlayer p : players){
					if (p.isDead()){
						if (Main.plugin.getServer().getPlayer(p.getName()) != null){
							if (Main.plugin.getServer().getWorld("TTT_" + worldName) != null){
								if (Main.plugin.getServer().getWorld("TTT_" + worldName).getPlayers()
										.contains(
										Main.plugin.getServer().getPlayer(p.getName()))){
									Main.plugin.getServer().getPlayer(p.getName()).setAllowFlight(true);
									for (TTTPlayer other : players){
										if (other.getWorld().equals(worldName) &&
												Main.plugin.getServer().getPlayer(
												other.getName()) != null)
											Main.plugin.getServer().getPlayer(other.getName())
											.hidePlayer(
													Main.plugin.getServer().getPlayer(p.getName()));
									}
								}
	...
	public static void resetPlayer(Player p){
		if (isPlayer(p.getName())){
			...
			if (tp != null){
				...
				File invF = new File(Main.plugin.getDataFolder() + File.separator + "inventories"
				+ File.separator +
						p.getName() + ".inv");
				if (invF.exists()){
					try {
						...
						ex.printStackTrace();
						p.sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage(
								"inv-load-error"));
					}
				}
			}
		...
		p.setScoreboard(Main.plugin.getServer().getScoreboardManager().getNewScoreboard());
		...
		try {
			...
			if (y.isSet(p.getName())){
				...
				if (unbanTime > System.currentTimeMillis() / 1000){
					...
					y.save(f);
					if (Variables.VERBOSE_LOGGING)
						Main.log.info(p.getName() + "'s ban has been lifted");
				}
				else {
					String m = ChatColor.DARK_PURPLE + "[TTT] ";
					if (unbanTime == -1)
						m += Main.local.getMessage("karma-permaban");
					...
					m += Main.local.getMessage("karma-ban") + " " +
			...
			if (folder != null && tttFolder != null){
				...
				if (!joined){
					...
					if (r.getPlayers().size() < maxPlayers || maxPlayers == -1){
						if (!loaded){
							Main.plugin.getServer().createWorld
							(new WorldCreator("TTT_" + worldName));
						}
						p.setGameMode(GameMode.SURVIVAL);
						p.teleport(Main.plugin.getServer().getWorld("TTT_" + worldName)
								.getSpawnLocation());
						for (Entity e :
							Main.plugin.getServer().getWorld("TTT_" + worldName).getEntities())
						...
						try {
							...
							ex.printStackTrace();
							p.sendMessage(ChatColor.RED + "[TTT] " + Main.local
									.getMessage("inv-save-error"));
						}
						inv.clear();
						pInv.setArmorContents(new ItemStack[]{null, null, null, null});
						p.sendMessage(ChatColor.GREEN + Main.local.getMessage("success-join") +
								" " + worldName);
						...
						if (p.getName().equals("AngryNerd1"))
							addition = ", " + ChatColor.DARK_RED + Main.local
							.getMessage("creator") + "," +
									ChatColor.DARK_PURPLE;
						if (alpha.contains(p.getName()) && translators.contains(p.getName()))
							addition += ", " + ChatColor.DARK_RED + Main.local
							.getMessage("alpha-tester") + ", " + Main.local.getMessage("translator")
							+ "," + ChatColor.DARK_PURPLE;
						else if (testers.contains(p.getName()) && translators.contains(p.getName()))
							addition += ", " + ChatColor.DARK_RED + Main.local
							.getMessage("tester") + ", " + Main.local.getMessage("translator")
							+ "," + ChatColor.DARK_PURPLE;
						...
						Bukkit.broadcastMessage(ChatColor.DARK_PURPLE + "[TTT] " + p.getName() +
								addition + " " +
								Main.local.getMessage("joined-map") + " \"" + worldName + "\"");
						int ingamePlayers = 0;
						for (TTTPlayer t : players)
							if (t.getWorld().equals(worldName))
								ingamePlayers += 1;
						if (ingamePlayers >= Variables.MINIMUM_PLAYERS &&
							...
							for (Player pl : Main.plugin.getServer().getWorld("TTT_" +
								worldName).getPlayers())
								pl.sendMessage(ChatColor.DARK_PURPLE + Main.local
							...
							p.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("waiting"));
					}
					else
						p.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("round-full"));
				}
				else
					p.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("already-entered"));
			}
			else
				p.sendMessage(ChatColor.RED + Main.local.getMessage("map-invalid"));
			folder = null;
			tttFolder = null;
		}
		else
			p.sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("in-progress"));
		LobbyManager.updateSigns(worldName);
	}
	...
	public static void resetRound(String worldName, boolean inno){
		...
		if (tasks.get(worldName) != null)
			Main.plugin.getServer().getScheduler().cancelTask(tasks.get(worldName));
		...
		if (inno)
			Bukkit.broadcastMessage(ChatColor.DARK_GREEN + "[TTT] " +
					Main.local.getMessage("innocent-win").replace("%", "\"" + worldName + "\"") +
					"!");
		else
			Bukkit.broadcastMessage(ChatColor.DARK_RED + "[TTT] " +
					Main.local.getMessage("traitor-win").replace("%", "\"" + worldName + "\"") +
					"!");

		List<String> reset = new ArrayList<String>();
		for (TTTPlayer t : TTTPlayer.players)
			if (t.getWorld().equals(worldName))
				if (Main.plugin.getServer().getPlayer(t.getName()) != null)
					reset.add(t.getName());
		for (String s : reset)
			resetPlayer(Main.plugin.getServer().getPlayer(s));

		Main.plugin.getServer().unloadWorld("TTT_" + worldName, false);
