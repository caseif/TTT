				...
				if (args[0].equalsIgnoreCase("import") || args[0].equalsIgnoreCase("i")){
					if (sender.hasPermission("ttt.import")){
						if (args.length > 1){
							...
							sender.sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("invalid-args-1"));
							sender.sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("usage-import"));
						}						
					}
					else
						sender.sendMessage(ChatColor.RED + Main.local.getMessage("no-permission-import"));
				}
					...
					if (sender instanceof Player){
						if (sender.hasPermission("ttt.join")){
							if (args.length > 1){
								...
								sender.sendMessage(ChatColor.RED + Main.local.getMessage("invalid-args-1"));
								sender.sendMessage(ChatColor.RED + Main.local.getMessage("usage-join"));
							}
						}
						else
							sender.sendMessage(ChatColor.RED + Main.local.getMessage("no-permission-join"));
					}
					else
						sender.sendMessage(ChatColor.RED + Main.local.getMessage("must-be-ingame"));
				}
						...
						if (sender.hasPermission("ttt.quit")){
							if (isPlayer(sender.getName())){
								sender.sendMessage(ChatColor.DARK_PURPLE + "[TTT] " + sender.getName() + " " +
										Main.local.getMessage("left-game").replace("%",
												getTTTPlayer(sender.getName()).getWorld()));
								RoundManager.resetPlayer((Player)sender);
							}
							else
								sender.sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("not-in-game"));
						}
						else
							sender.sendMessage(ChatColor.RED + "[TTT] " +
									Main.local.getMessage("no-permission-quit"));
					}
					else
						sender.sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("must-be-ingame"));
				}
					...
					if (sender.hasPermission("ttt.setspawn")){
						...
						sender.sendMessage(ChatColor.RED + Main.local.getMessage("no-permission"));
					...
					if (args.length > 1 && args[1].equalsIgnoreCase("lobby")){
						if (sender.hasPermission("ttt.lobby.create")){
							sender.sendMessage(ChatColor.GOLD + "" + ChatColor.UNDERLINE +
									Main.local.getMessage("lobby-help"));
							sender.sendMessage("");
							sender.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("first") + " " +
									Main.local.getMessage("line") + " " +
									ChatColor.GREEN + "[TTT]");
							sender.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("second") + " " +
									Main.local.getMessage("line") + " " +
									ChatColor.GREEN + Main.local.getMessage("type"));
							sender.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("third") + " " +
									Main.local.getMessage("line") + " " +
									ChatColor.GREEN + Main.local.getMessage("round"));
							sender.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("fourth") + " " +
									Main.local.getMessage("line") + " " +
									ChatColor.GREEN + Main.local.getMessage("number"));
						}
						else
							sender.sendMessage(Main.local.getMessage("no-permission"));
					}
					else if (sender.hasPermission("ttt.help")){
						sender.sendMessage(ChatColor.GOLD + "" + ChatColor.UNDERLINE +
								Main.local.getMessage("commands"));
						sender.sendMessage("");
						if (sender.hasPermission("ttt.join"))
							sender.sendMessage(ChatColor.DARK_PURPLE + "/ttt join, j " + ChatColor.GREEN +
									Main.local.getMessage("join-help"));
						if (sender.hasPermission("ttt.quit"))
							sender.sendMessage(ChatColor.DARK_PURPLE + "/ttt quit, q " + ChatColor.GREEN +
									Main.local.getMessage("quit-help"));
						if (sender.hasPermission("ttt.import"))
							sender.sendMessage(ChatColor.DARK_PURPLE + "/ttt import, i " + ChatColor.GREEN +
									Main.local.getMessage("import-help"));
						if (sender.hasPermission("ttt.setspawn"))
							sender.sendMessage(ChatColor.DARK_PURPLE + "/ttt setspawn, ss " + ChatColor.GREEN +
									Main.local.getMessage("spawn-help"));
						if (sender.hasPermission("ttt.help"))
							sender.sendMessage(ChatColor.DARK_PURPLE + "/ttt help, ? " + ChatColor.GREEN +
									Main.local.getMessage("help-help"));
					}
					else
						sender.sendMessage(ChatColor.DARK_PURPLE + Main.local.getMessage("no-permission"));
				}
				else {
					sender.sendMessage(ChatColor.RED + "[TTT] " + Main.local.getMessage("invalid-args-2"));
					sender.sendMessage(ChatColor.RED + Main.local.getMessage("usage-1"));
				}
			}
			else {
				sender.sendMessage(ChatColor.RED + Main.local.getMessage("invalid-args-1"));
