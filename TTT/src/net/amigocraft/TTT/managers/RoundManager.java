...
public class RoundManager {
	...
	public void gameTimer(final String worldName){
		plugin.tasks.put(worldName, plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable(){
			...
			public void run(){
				...
				if (!(tLeft && iLeft)){
					...
					plugin.getServer().getScheduler().cancelTask(plugin.tasks.get(worldName));
					plugin.tasks.remove(plugin.tasks.get(worldName));
					...
					if (newTime % 60 == 0 && newTime >= 60){
						...
						Bukkit.getScheduler().cancelTask(plugin.tasks.get(worldName));
						plugin.getServer().unloadWorld("TTT_" + worldName, false);
						new WorldUtils().rollbackWorld(worldName);
					}
				}
